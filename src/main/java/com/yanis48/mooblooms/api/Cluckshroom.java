...
public class Cluckshroom {
	...
	private Cluckshroom(Cluckshroom.Builder settings) {
		...
		FabricEntityTypeBuilder<?> builder = FabricEntityTypeBuilder.create(SpawnGroup.CREATURE, CluckshroomEntity::new).dimensions(EntityDimensions.fixed(0.4F, 0.7F));
		if (this.fireImmune) {
			builder.fireImmune();
		}
		...
		EntityRendererRegistry.INSTANCE.register(this.entityType, (dispatcher, context) -> new CluckshroomEntityRenderer(dispatcher));
