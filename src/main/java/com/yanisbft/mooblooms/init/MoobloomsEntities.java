...
public class MoobloomsEntities {
	...
	private static Predicate<BiomeSelectionContext> lushCaves = BiomeSelectors.includeByKey(BiomeKeys.LUSH_CAVES);
	...
	public static final Moobloom AZALEA_MOOBLOOM = new Moobloom.Builder().name(Mooblooms.id("azalea_moobloom")).blockState(Blocks.FLOWERING_AZALEA.getDefaultState()).blockStateRenderer(-0.5F, -0.5F, 0.5F, -0.5D, -1.0D, -0.5D).spawnEntry(new SpawnEntry(lushCaves, Mooblooms.config.azaleaMoobloom)).spawnEgg(0x70922D, 0xBA62CE).configCategory(Mooblooms.config.azaleaMoobloom).build();
	public static final Moobloom DRIPLEAF_MOOBLOOM = new Moobloom.Builder().name(Mooblooms.id("dripleaf_moobloom")).blockState(Blocks.SMALL_DRIPLEAF.getDefaultState().with(SmallDripleafBlock.HALF, DoubleBlockHalf.UPPER)).blockStateRenderer(-0.6F, -0.6F, 0.6F, -0.5D, -0.8D, -0.5D).spawnEntry(new SpawnEntry(lushCaves, Mooblooms.config.dripleafMoobloom)).spawnEgg(0x42552D, 0x98C440).cannotPlaceBlocks().configCategory(Mooblooms.config.dripleafMoobloom).build();
