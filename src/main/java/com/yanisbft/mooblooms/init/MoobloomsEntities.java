...
public class MoobloomsEntities {
	...
	public static final Moobloom WITHER_ROSE_MOOBLOOM = new Moobloom.Builder().name(Mooblooms.id("wither_rose_moobloom")).blockState(Blocks.WITHER_ROSE).fireImmune().ignoredEffects(List.of(StatusEffects.WITHER.value())).spawnPredicate(AnimalWithBlockState::canSpawnNether).particle(ParticleTypes.SMOKE).spawnEntry(new SpawnEntry(Mooblooms.config.witherRoseMoobloom)).spawnEgg(0x2A1F19, 0x636363).configCategory(Mooblooms.config.witherRoseMoobloom).build();
	...
	public static final Moobloom EYEBLOSSOM_MOOBLOOM = new Moobloom.Builder().name(Mooblooms.id("eyeblossom_moobloom")).blockState(world -> (world.isNight() ? Blocks.OPEN_EYEBLOSSOM : Blocks.CLOSED_EYEBLOSSOM).getDefaultState()).validBlocks(List.of(Blocks.GRASS_BLOCK, Blocks.MOSS_BLOCK)).spawnEntry(new SpawnEntry(Mooblooms.config.eyeblossomMoobloom)).spawnGroup(SpawnGroup.AMBIENT).spawnPredicate(AnimalWithBlockState::canSpawn).spawnEgg(0x8F8288, 0x3D3636).configCategory(Mooblooms.config.eyeblossomMoobloom).build();
	public static final Moobloom AZALEA_MOOBLOOM = new Moobloom.Builder().name(Mooblooms.id("azalea_moobloom")).blockState(Blocks.FLOWERING_AZALEA).blockStateRenderer(-0.5F, -0.5F, 0.5F, -0.5D, -1.0D, -0.5D).spawnEntry(new SpawnEntry(Mooblooms.config.azaleaMoobloom)).spawnGroup(SpawnGroup.AMBIENT).spawnPredicate(AnimalWithBlockState::canSpawnLushCaves).spawnEgg(0x70922D, 0xBA62CE).configCategory(Mooblooms.config.azaleaMoobloom).build();
	public static final Moobloom DRIPLEAF_MOOBLOOM = new Moobloom.Builder().name(Mooblooms.id("dripleaf_moobloom")).blockState(Blocks.SMALL_DRIPLEAF.getDefaultState().with(SmallDripleafBlock.HALF, DoubleBlockHalf.UPPER)).blockStateRenderer(-0.6F, -0.6F, 0.6F, -0.5D, -0.8D, -0.5D).spawnEntry(new SpawnEntry(Mooblooms.config.dripleafMoobloom)).spawnGroup(SpawnGroup.AMBIENT).spawnPredicate(AnimalWithBlockState::canSpawnLushCaves).spawnEgg(0x42552D, 0x98C440).cannotPlaceBlocks().configCategory(Mooblooms.config.dripleafMoobloom).build();
