...
public class MoobloomsEntities {
	...
	public static final Moobloom WITHER_ROSE_MOOBLOOM = new Moobloom.Builder().name(Mooblooms.id("wither_rose_moobloom")).blockState(Blocks.WITHER_ROSE).fireImmune().ignoredEffects(ImmutableList.of(StatusEffects.WITHER.value())).spawnPredicate(AnimalWithBlockState::canSpawnNether).particle(ParticleTypes.SMOKE).spawnEntry(new SpawnEntry(Mooblooms.config.witherRoseMoobloom)).spawnEgg(0x2A1F19, 0x636363).configCategory(Mooblooms.config.witherRoseMoobloom).build();
